class Controller {
  ...
  async criaRegistroController(req, res) {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ message: `erro ao criar, mensagem do erro:${e}` });
    }
  }
  ...
  async criaVariosRegistros(req, res) {
    ...
    try {
      if (isTrue.status) {
        const novoRegistroCriado = await this.propsServices.criaVarios(
          dadosParaCriacao
        );
        return res.status(200).json(novoRegistroCriado);
      } else {
  ...
  async pegaTodosController(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: `erro ao buscar registro, mensagem do erro: ${e}` });
    }
  }
  //-------------------------------------READ-POR-ID-------------------------------------//
  ...
  async pegaUmRegistroPorIdController(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({
          message: `erro ao buscar registro, mensagem do erro: ${erro}`,
        });
    }
  }
  ...
  async excluiRegistroController(req, res) {
    ...
    try {
      ...
      if (foiDeletado === 0) {
        return res
          .status(400)
          .json({ message: `id ${id} não encontrado`, resposta: foiDeletado });
      } else {
        return res
          .status(200)
          .json({ message: `id ${id} deletado`, resposta: foiDeletado });
      }
    } catch (error) {
